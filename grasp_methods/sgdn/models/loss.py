...
def focal_loss(net, x, y_pts, y_ang, y_wid):
    ...
    # 置信度损失
    able_pred = torch.sigmoid(able_pred)
    # able_loss = bin_focal_loss(able_pred, y_pts, width=False) * 10
    able_loss = bin_focal_loss(able_pred, y_pts, width=True, alpha=0.95) * 10

    # 抓取角损失
    angle_pred = torch.sigmoid(angle_pred)
    # angle_loss = bin_focal_loss(angle_pred, y_ang, width=False) * 10
    angle_loss = bin_focal_loss(angle_pred, y_ang, width=True, alpha=0.95) * 10

    # 抓取宽度损失
    width_pred = torch.sigmoid(width_pred)
    width_loss = bin_focal_loss(width_pred, y_wid, width=True, alpha=0.95) * 20
