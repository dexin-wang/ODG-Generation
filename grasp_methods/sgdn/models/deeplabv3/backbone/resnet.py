...
class ResNet(nn.Module):
    def __init__(self, block, input_channels, layers, output_stride, batchNorm, pretrained=True):
        """
    ...
    def _load_pretrained_model(self):
        ...
        if self.device == 'cpu':
            pretrain_dict = torch.load('D:/research/grasp_detection/SGDN/代码/sgdn-rgb-2/NN/resnet101-5d3b4d8f.pth')
        else:
            pretrain_dict = torch.load('/home/wangdx/research/grasp_detection/sgdn-rgb-2/NN/resnet101-5d3b4d8f.pth')
...
def ResNet101(output_stride, input_channels, BatchNorm, pretrained=True):
    ...
    """
    model = ResNet(Bottleneck, input_channels, [3, 4, 23, 3], output_stride, BatchNorm, pretrained=pretrained)
    return model

def ResNet50(output_stride, input_channels, BatchNorm, pretrained=True):
    """Constructs a ResNet-50 model.
    Args:
        output_stride: 16
    """
    model = ResNet(Bottleneck, input_channels, [3, 4, 6, 3], output_stride, BatchNorm, pretrained=pretrained)
    return model
